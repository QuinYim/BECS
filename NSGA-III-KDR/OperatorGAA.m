function Offspring = OperatorGAA(Problem,Parent,Parameter)
...
%--------------------------------------------------------------------------
    ...
    % if nargin > 2
    %     [disC,disM] = deal(Parameter{:});
    % else
    %     [disC,disM] = deal(20,20);
    % end

    %proM=0.25*(0.08+(Problem.maxFE-Problem.FE)*0.92/Problem.maxFE);
    %proM=0.25*(0.08+Problem.FE*0.92/Problem.maxFE);
    %proM=0.5;
    [disC,disM] = deal(20,20);
    proM = ((exp(1) + exp(-1))/(exp(1) - exp(-1))) * tanh(((Problem.maxFE/Problem.N)-(Problem.FE/Problem.N))/(Problem.maxFE/Problem.N));
    proC = ((exp(1) + exp(-1))/(exp(1) - exp(-1))) * tanh(((Problem.maxFE/Problem.N)-(Problem.FE/Problem.N))/(Problem.maxFE/Problem.N));
...
% Genetic operators for real and integer variables
    ...
    %% Simulated binary crossover二进制交叉
    [N,D] = size(Parent1);
    beta  = zeros(N,D);
    mu    = rand(N,D);
